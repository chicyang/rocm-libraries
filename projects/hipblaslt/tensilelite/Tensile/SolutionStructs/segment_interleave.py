...
def _coarse_a(state):
    # A must cover a full component (never crosses one) -- A is the de-conflicted operand, so its
    # read must land cleanly in a single segment. Equivalent to VWA == WaveTileA.
    numComp = state["NumWaves"] // 2
...
def evaluate(state):
    ...
    if _force_bcontig or not _b_readable(state):
        ...
        # WaveTileB) or when forced. Only A is de-conflicted; B keeps its normal layout.
